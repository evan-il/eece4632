...
typedef struct {
    ...
    XBlur_edge_uio_map maps[ MAX_UIO_MAPS ];
} XBlur_edge_uio_info;

/***************** Variable Definitions **************************************/
static XBlur_edge_uio_info uio_info;
...
static int uio_info_read_name(XBlur_edge_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XBlur_edge_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XBlur_edge_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XBlur_edge_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XBlur_edge_Initialize(XBlur_edge *InstancePtr, const char* InstanceName) {
	XBlur_edge_uio_info *InfoPtr = &uio_info;
